...
Partition layout (applied to both SD Card 0 and SD Card 1):
  1. Housekeeping  ->  3 GB   (/hk)
  2. Logs          ->  3 GB   (/logs)
  3. Software      ->  10 GB  (/fsw)
  4. Iris          ->  10 GB  (/iris)
  5. DFGM          ->  6 GB   (/dfgm)

SD Card 0 mounts under /mnt/storage/<partition>
SD Card 1 mounts under /mnt/backup/<partition>

Usage:
  sudo python3 format_exalta3_storage.py [--device /dev/sdX] [--role primary|backup]
...
def select_role(args_role):
    ...
    print("  [1] Primary storage  (SD Card 0, mounts under /mnt/storage)")
    print("  [2] Backup storage   (SD Card 1, mounts under /mnt/backup)")
    while True:
...
def create_partitions(device):
    ...
    for i, part in enumerate(PARTITIONS, start=1):
        ...
        print(f"\n  Partition {i}: {part['name']}  ({part['size_gb']} GB)")
        run([
            "parted", "-s", device, "mkpart",
            part["name"],   # partition name (GPT label)
            "ext4",
            f"{start_mb}MiB",
            f"{end_mb}MiB",
        ])
...
def format_partitions(device):
    ...
    for i, part in enumerate(PARTITIONS, start=1):
        ...
        print(f"\n  Formatting {node} as ext4 (label: {part['label']})...")
        run([
            "mkfs.ext4",
            "-L", part["label"],   # filesystem label
...
def create_mount_points(role):
    ...
    print(f"\nCreating mount points under {mount_root} ...")
    for part in PARTITIONS:
        mp = os.path.join(mount_root, part["label"])
...
def print_summary(device, role):
    ...
    print(f"  Role   : {role}")
    print()
    print(f"  {'#':<4} {'Partition':<16} {'Label':<10} {'Size':>6}  {'Mount Point'}")
    print("  " + "-" * 58)
    for i, part in enumerate(PARTITIONS, start=1):
        node = partition_node(device, i)
        mp = f"{mount_root}/{part['label']}"
        print(f"  {i:<4} {part['name']:<16} {part['label']:<10} {part['size_gb']:>4} GB  {mp}")
    print()
...
Examples:
  ...
  sudo python3 format_exalta3_storage.py --device /dev/mmcblk0 --role backup
    ...
    parser.add_argument(
        ...
        help="Card role: 'primary' (SD Card 0, /mnt/storage) or 'backup' (SD Card 1, /mnt/backup).",
    )
    parser.add_argument(
        "--skip-mount-points",
        action="store_true",
        help="Skip creation of mount point directories.",
    )
...
def main():
    ...
    # 6. Mount points
    if not args.skip_mount_points:
        create_mount_points(role)

    # 7. Done
    print_summary(device, role)
    print_fstab_entries(device, role)
